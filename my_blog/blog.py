...
__all__ = ['AppState', 'create_database_tables', 'create_post_database', 'create_app', 'intro', 'hx_attrs', 'hx_link', 'navbar',
           'layout', 'index', 'about', 'blog', 'x_icon', 'social_link', 'footer', 'slug_exists', 'add_post', 'blogpost',
           'get_post_tags', 'get_posts', 'tag_pill', 'tag_filter', 'tag_badge', 'load_thumbnail', 'get_post_image',
           'post_card', 'process_upload', 'get', 'post', 'rewrite_image_paths']
...
# %% ../nbs/01_blog_v2.ipynb #3946a1bd
def tag_badge(name):
    return Span(name, cls="text-xs px-2 py-1 rounded bg-muted")

# %% ../nbs/01_blog_v2.ipynb #c8f4649c
@rt
def blog(htmx, tags:str=None):
    # selected is a SET of the name of the selected tags
    selected = {unquote(t.strip()) for t in (tags or '').split(',') if t.strip()}
